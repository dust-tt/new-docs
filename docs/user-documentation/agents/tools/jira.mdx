...
The Jira tool is a powerful integration that allows users to seamlessly manage Jira projects and issues directly from within Dust. This feature enables automated ticket management, project tracking, and workflow transitions without leaving the Dust environment.
...
Once the tool has been configured by the admin as described before, it can be selected on any agent: in the Agent Builder, simply click on `Add Tool` and select Jira.
