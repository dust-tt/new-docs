...
**Tools** enable you to enhance an agent with various abilities, allowing it to efficiently tackle complex scenarios by simultaneously carrying out multiple tasks.
...
<Tip>
  Use instructions to "script" your agent. It is crucial not to overestimate the
...
## What use cases can Capabilities unlock?
...
Please refer to our [Developer Platform overview](https://docs.dust.tt/reference/developer-platform-overview) if you want to learn more about Dust Apps.
...
Workspace-wide tools (those in Company Space) won't appear in individual spaces' "Add Tools" dropdown, this is expected behavior. The tool IS accessible to all users; it simply doesn't need to be added again.
