...
The Image Generation capability uses the **gemini-3-pro-image** model to turn your text descriptions into high-quality images. Your agents can create product photography pipelines, brand asset variations, multi-format marketing campaigns, and visual style transfers on demand.
...
Agents can run multiple image generations **in parallel** and use **reference images** from previous outputs. This lets you build creative pipelines that maintain consistency while producing multiple assets efficiently.
