...
You can pull usage data programmatically through the [Export workspace analytics](/api-reference/get_api-v1-w-wid-analytics-export) endpoint. Any Dust API key with the **builder** role (or above) can call it — no special access request needed.
...
The previous [`workspace-usage`](/api-reference/get_api-v1-w-wid-workspace-usage) endpoint is deprecated and will be removed after **2026-06-01**. Migrate to `analytics/export` before then.
</Danger>
