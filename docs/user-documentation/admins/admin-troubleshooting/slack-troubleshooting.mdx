...
Inviting the bot makes the channel visible in Dust's connection manager. But messages only sync after you explicitly select that channel in **Spaces  Connections  Slack  Add / Remove data** (:doc:`/docs/user-documentation/data-sources/connections`). This gives you control over exactly what data enters Dust.
</Info>
...
2. In Dust, go to **Spaces  Connections  Slack  Add / Remove data** (see [Slack Connection](https://docs.dust.tt/docs/docs/user-documentation/data-sources/connections))
...
2. Copy the manifest exactly as provided in the Dust docs—do not edit it (:doc:`/docs/user-documentation/data-sources/connections`)
...
**If you rotated credentials for security reasons:** Update them immediately in **Spaces  Connections  Slack  Manage  Edit** (:doc:`/docs/user-documentation/data-sources/connections#managing-permissions`). Enter the new credentials from your Slack app's Basic Information page. The connection should resume automatically.

### Legacy connection migration

**Why this matters:** Slack connections created before December 2024 use a shared Dust app. These connections work until **March 3, 2026**, but must be migrated to your own Slack app before that deadline (:doc:`/docs/user-documentation/data-sources/connections`).
...
**How to fix it:** Check **Spaces  Connections  Slack** to see access restrictions and which spaces receive which data (:doc:`/docs/user-documentation/data-sources/connections#managing-permissions`).
...
| **Bot messages not synced**                | Messages from `@Dust` and other bots are intentionally excluded from sync. This prevents AI‑generated content from being treated as source material, which could propagate mistakes. (:doc:`/docs/user-documentation/data-sources/connections#data-synchronization`) |
...
| **Thread links sometimes approximate**     | When messages aren't threaded, Dust links to the approximate location in the channel rather than the exact message. Use threads for precise linking. (:doc:`/docs/user-documentation/data-sources/connections#slack-connection-specificities`)                       |

<Info>
**Best practice: Use threads**

Post your topic as the first message, then continue the conversation in a thread. This improves Dust's ability to retrieve relevant context and provides accurate message links (:doc:`/docs/user-documentation/data-sources/connections#slack-connection-specificities`).
</Info>
