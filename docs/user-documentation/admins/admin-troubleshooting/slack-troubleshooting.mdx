...
description: "Dust's Slack integration has two layers: **Slack Apps** and **Features**. A Slack app is the foundation. When you install a Slack app, it creates a bo..."
---
...
<Callout icon="" theme="default">
  ...
  Each app has a distinct job. **Dust Data Sync** reads and indexes your messages so agents can search them later (see [Slack Connection](https://docs.dust.tt/docs/slack-connection)). **Dust** is the interactive bot that responds when you mention it (see [Dust in Slack](https://docs.dust.tt/docs/slack)). Separating these keeps permissions clear and lets you control each independently.
</Callout>
...
<Callout icon="" theme="default">
  ...
  Inviting the bot makes the channel visible in Dust's connection manager. But messages only sync after you explicitly select that channel in **Spaces  Connections  Slack  Add / Remove data** (:doc:`/slack-connection`). This gives you control over exactly what data enters Dust.
</Callout>
...
2. In Dust, go to **Spaces  Connections  Slack  Add / Remove data** (see [Slack Connection](https://docs.dust.tt/docs/slack-connection))
...
* If your connection was set up before December 2024, follow the [Slack Connection setup guide](/slack-connection) to create your own Slack App.
* To enable the interactive bot, go to **Workspace Management  Workspace Settings  Integrations** and toggle Slack on (:doc:`/slack`).
...
1. In Dust, go to **Workspace Management  Workspace Settings  Integrations** and verify Slack is toggled on (:doc:`/slack`).
...
<Callout icon="" theme="default">
  ...
  `@Dust` works in public and private channels only. It does not respond in direct messages (:doc:`/slack`). This is by design—channels provide shared context that makes agent responses more useful to everyone.
</Callout>
...
**For public channels:** You can set a default agent in the agent's settings (the agent must be published). When anyone mentions `@Dust` in that channel, your chosen agent responds automatically (:doc:`/slack`).

<Callout type="warning"Private channel limitation>
</Callout>
...
1. **Agent settings:** Configure a default agent for specific public channels in the agent's settings page (:doc:`/slack`).
2. **Slack Workflows:** Use [Slack Workflows](https://docs.dust.tt/docs/dust-assistants-in-a-slack-workflow) to trigger agents based on events like emoji reactions or schedules.

<Callout type="warning"Workflows require whitelisting>
</Callout>
Slack workflows must be enabled for your workspace by Dust. Contact [support@dust.tt](mailto:support@dust.tt) with your workspace URL and workflow name to request access (see [Slack Workflows](https://docs.dust.tt/docs/dust-assistants-in-a-slack-workflow)). When you contact support, include the exact name(s) of any Restricted Space used by the agent (in addition to Company Data). This is indicated by the yellow Restricted Space heads‑up in the Agent Builder.
...
| ---------------------------- | ----------------------- | -------------------------------------------------- |
| **Semantic Search Messages** | Slack AI add‑on enabled | Uses natural language understanding—better results |
...
**Why this happens:** Slack tools use your personal OAuth credentials. When an agent posts a message or performs an action, Slack sees it as coming from you. This is intentional—it means agents can only do what you're allowed to do (:doc:`/slack-mcp`).

**If you need bot identity:** Contact [support@dust.tt](mailto:support@dust.tt) to request access to the **Slack Bot** tool, which posts as a bot instead of you. Note: the bot version only works in public channels where the bot has been invited (:doc:`/beta-slack-tool-using-bot-credentials`).
...
4. The Slack tools now appear when building agents (:doc:`/slack-mcp`).
...
2. Copy the manifest exactly as provided in the Dust docs—do not edit it (:doc:`/slack-connection`)
...
**If you rotated credentials for security reasons:** Update them immediately in **Spaces  Connections  Slack  Manage  Edit** (:doc:`/slack-connection#managing-permissions`). Enter the new credentials from your Slack app's Basic Information page. The connection should resume automatically.

### Legacy connection migration

**Why this matters:** Slack connections created before December 2024 use a shared Dust app. These connections work until **March 3, 2026**, but must be migrated to your own Slack app before that deadline (:doc:`/slack-connection`).

**How to fix it:** Follow the [Slack Connection setup guide](/slack-connection). The process takes about 2 minutes and involves creating your own Slack app from a manifest.
...
**How to fix it:** Check **Spaces  Connections  Slack** to see access restrictions and which spaces receive which data (:doc:`/slack-connection#managing-permissions`).
...
| **Bot messages not synced**                | Messages from `@Dust` and other bots are intentionally excluded from sync. This prevents AI‑generated content from being treated as source material, which could propagate mistakes. (:doc:`/slack-connection#data-synchronization`) |
...
| **Thread links sometimes approximate**     | When messages aren't threaded, Dust links to the approximate location in the channel rather than the exact message. Use threads for precise linking. (:doc:`/slack-connection#slack-connection-specificities`)                       |

<Callout icon="" theme="default">
</Callout>
  ### Best practice: Use threads

  Post your topic as the first message, then continue the conversation in a thread. This improves Dust's ability to retrieve relevant context and provides accurate message links (:doc:`/slack-connection#slack-connection-specificities`).
</Callout>
