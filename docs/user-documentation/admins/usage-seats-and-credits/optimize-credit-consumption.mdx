...
description: "Credit optimization is not about using Dust less. It is about making sure each agent does the right amount of work for the value it creates. As your t..."
---
...
Workspace admins can review adoption and usage patterns from **Admin > Analytics**. The dashboard includes workspace activity over time, message sources, tool usage, skill usage, top users, and top agents. Admins can scope the dashboard to the last 7, 14, 30, or 90 days. See [Workspace analytics](././workspace-analytics) for details.
...
Workspace admins can review programmatic credit consumption in **Admin > Usage** under the Programmatic usage settings. For details on how programmatic usage draws credits and how to configure the monthly cap, see [Credit management](././credit-management).
...
[Go Deep](././go-deep) gives agents the ability to conduct more comprehensive research across company data, databases, and web sources. It includes capabilities such as sub-agents, company data exploration, data warehouse queries, and web search and browsing.
...
Dust provides [context compaction](././context-compaction) to help long conversations continue with summarized context instead of replaying the full earlier history. The original messages remain stored and visible, but future agent runs use the compaction summary and the messages after the compaction marker.
