---
title: "Train Team Credit Optimization"
description: "--- title: Train your team on credit optimization excerpt: How to build the habits, framing, and workflows that make AI usage efficient at scale. depr..."
---
...
These prompts produce better output. They tell the agent what work matters, where to look, how much depth to use, and when to stop. See [How to write effective instructions](././prompting-101-how-to-talk-to-your-agents) for more guidance.
...
[Go Deep](././go-deep) gives agents the ability to conduct comprehensive research across company data, databases, and web sources. It includes sub-agents, company data exploration, data warehouse queries, and web search and browsing.
...
[Skills](././skills) in Dust are reusable packages of instructions, knowledge, and tools that you can share across multiple agents. When you update a skill's instructions, every agent using that skill automatically gets the improvement.
...
For builders, credit optimization is product design. Every agent has a usage profile. Every model choice, data source, tool, and instruction changes that profile. See [How to write effective instructions](././prompting-101-how-to-talk-to-your-agents), [model settings](././what-settings-model-should-i-use), and [agent troubleshooting](././how-to-optimize-your-dust-agent) for details.
...
- Write clear, specific instructions — vague instructions make agents do more guessing. See [How to write effective instructions](././prompting-101-how-to-talk-to-your-agents).
- Test model changes on real examples before changing an important workflow.
- Use [skills](././skills) to share instructions and tools across multiple agents.
...
| Cadence       | What to do                                                                                                                                                                                                                                         |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Weekly**    | Admins review the shape of usage: top agents, top sources, programmatic growth, expensive model usage, deep research usage, and sudden changes. See [Workspace analytics](././workspace-analytics) and [Credit management](././credit-management). |
...
| **Automation checklist**    | Owner, expected volume, test batch, cost per run, failure behavior, and monitoring. See [trigger rate limits](././rate-limiting). |
