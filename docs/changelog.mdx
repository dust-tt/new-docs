...
## Personal Zendesk accounts

What is it?

Users can connect their own Zendesk account to the Zendesk tool, alongside the workspace-level connection. The Zendesk subdomain is inherited from the workspace connection, so users do not need to enter it again.

Why is it useful?

Actions in Zendesk run as the user's own agent identity instead of through a shared workspace account.

How to access it?

Open the Zendesk tool connection settings in a workspace where Zendesk is configured, then connect your personal Zendesk account. Available now in all workspaces with the Zendesk tool configured.
