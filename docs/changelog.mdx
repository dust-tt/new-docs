...
## Personal account support for the Zendesk tool

What is it?

Users can now connect their own personal Zendesk account to the Zendesk tool, alongside the existing workspace-level connection. The Zendesk subdomain is automatically inherited from the admin’s workspace connection - so users never have to re-enter it. Mirrors the existing Jira personal auth pattern.

Why is it useful?

So users can act in Zendesk as themselves (their own agent identity), rather than going through a single shared workspace account.

How does it work?

Zendesk can use either the workspace-level connection or a user's personal connection. The Zendesk subdomain is inherited from the workspace connection, so users do not need to enter it again.

How to access it?

In a workspace where the Zendesk tool is configured, connect your personal Zendesk account when setting up the tool. This feature is live for all such workspaces and does not require a feature flag.
...
Youtrust is available as a Remote MCP Server, which is simply a secure way for Dust to connect to an external service. The connection uses standard, secure authentication (OAuth), so your agents can act on your Youtrust account safely once it's set up.
...
- **Empower your team** to interact with Monday.com through natural language
...
Until now, Dust could connect to Confluence as a read-only data source — syncing pages so agents could search them. With the MCP Server, agents go further: they can now act on Confluence in real time. Whether it's fetching the latest version of a page, pulling a comment thread, or drafting and publishing new content, your agents become active contributors to your knowledge base, not just readers.
...
- **Documentation Q\&A**: Ask "What does our runbook say about database rollbacks?" — the agent searches Confluence in real time.
...
Save time on repetitive event management tasks, reduce context-switching, and empower your team to get event information or take action without needing direct Luma access.
...
Once configured by an admin, the Snowflake MCP Server is available as a tool that any agent can use. Simply ask your agent a data-related question, and it will query Snowflake, retrieve the relevant results, and surface them directly in the conversation.

✨ **Concrete Use Cases**

- **Sales reporting**: Ask your agent "What were our top 10 accounts by revenue last quarter?" — it queries Snowflake and returns the answer instantly.
...
This is available to everyone, right now. Simply ask an agent to build a Frame and request an export — the agent will handle the rest.
...
Context Compaction is now available to all users. Simply watch for the context usage indicator in your input bar—when it appears, you'll be guided through the process automatically. Learn more in our [documentation](https://docs.dust.tt/docs/context-compaction).
...
Steering transforms how you interact with Dust agents by allowing you to send messages _while the agent is working_. You can now see every step of the agent's work in real-time—thinking, tool calls, searches—and redirect it on the fly without canceling or losing progress. Additionally, messages are now scoped to one agent at a time, displayed in the input bar, so you no longer need to use @mentions.
...
Write access is available to all users who already have the Salesforce MCP tool configured in their workspace. Simply use it in your existing agents — the new capabilities are available now.
...
You can now interact with your Dust agents directly via email. Simply send or forward an email to `agent-name@dust.team` and your agent will process it and reply back to you. This works just like talking to an agent in Slack or the web app, but right from your inbox.
...
The in-app chart gives you a visual overview of your spending, but deeper analysis — filtering by team, building custom reports, or combining with other data — is easier when you have the raw data. CSV export unlocks that without any additional setup.
...
The upgrade is already live in your Dust workspace. Simply use the image generation feature as usual—the new capabilities are automatically available. For inspiration and detailed examples, check out our documentation: [https://docs.dust.tt/docs/image-generation](https://docs.dust.tt/docs/image-generation)
...
**Domain expertise**: Package your team's specialized knowledge—like "Financial Analysis," "Legal Compliance," or "Technical Support"—so any agent can leverage expert-level capabilities without you rebuilding the context each time.
...
- **Effortless updates**: Improve all your agents at once by updating a single Skill
...
- Mentions work seamlessly within the agent configuration interface
</Update>
...
- Enhanced visual editing capabilities for composing messages
</Update>
...
You can now insert emojis in your messages using the `:emoji_name:` syntax. Simply type any supported emoji shortcode (like `:smile:` or `:rocket:`) and it will automatically convert to the corresponding emoji.
...
• Configure webhooks directly in Spaces settings
• Seamlessly integrates with GitHub MCP Server for complete automation workflows
...
• Enhanced list channels functionality with reduced payload size to optimize LLM context usage
...
• Streamlined Excel workbook automation capabilities
...
• Added new Microsoft Teams MCP server with comprehensive tools
...
• Agents can now perform comprehensive document workflows across Microsoft platforms
...
• Enhanced assistant capabilities for more powerful interactions
...
• Added a new Go Deep Tool that allows agents to hand off complex tasks for comprehensive analysis
• Tool is available by default in @Dust agent
• Can be added to any custom agent through the agent builder
• Enables specialized agents to leverage deep-dive capabilities while maintaining awareness of main agent instructions
...
• Explores topics iteratively before producing comprehensive answers
...
• Enhanced `search` tool with folder-scoped searching capabilities
...
• Build, deploy, and manage Val Town scripts seamlessly
...
• Enhanced the existing semantic search capabilities with write actions
...
• Enables seamless handling of lengthy content without disrupting message flow
...
• Personal attribution makes collaboration more seamless and audit trails clearer
• Available to all users with no feature flag required
...
• Access Dust functionality seamlessly within your Excel workflows
...
• Comment on issues and manage workflows seamlessly
...
- Integrated thread context handling for seamless email management
</Update>
...
- Seamless integration with spreadsheet data
</Update>
...
- Enhanced search capabilities with owner filtering
...
- Interact with Slack workspace content seamlessly
</Update>
...
- Enhanced sharing controls and permissions
...
- Seamless connection to sync Gong call transcripts
...
- Added comprehensive solutions pages featuring 10 core functions
...
- Enhanced assistant autocomplete and search functionality
...
• Integrates seamlessly with existing Assistant tools and instructions\
...
• Enhanced SSO workspace integration\
...
• Streamlined input bar animation for smoother transitions
...
• Added comprehensive documentation to help deploy Dust within organizations\
...
• Access Dust features seamlessly while browsing
...
- Enhanced performance and reliability of the `DUST()` function
</Update>
...
- Streamlined responses for improved clarity
...
- Enhances spreadsheet functionality with AI support
...
- Enables the creation of graphs and charts for enhanced data analysis
...
• Streamlines the process of sharing files within conversations
...
• Enhances mathematical and scientific communication
